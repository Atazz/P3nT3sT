...
````bash  
Ryohei@htb[/htb]$ sudo mkdir -p /var/www/uploads/SecretUploadDirectory
````
2. Change the owner to www-data.
````bash  
Ryohei@htb[/htb]$ sudo chown -R www-data:www-data /var/www/uploads/SecretUploadDirectory
````
...
````bash 
Ryohei@htb[/htb]$ sudo ln -s /etc/nginx/sites-available/upload.conf /etc/nginx/sites-enabled/
````
5. Start nginx.
`````bash  
Ryohei@htb[/htb]$ sudo systemctl restart nginx.service
````
...
````bash
Ryohei@htb[/htb]$ sudo rm /etc/nginx/sites-enabled/default
````
